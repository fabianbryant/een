...
class ApiActions(object):
    ...
    def get_list_bridges(self):
        ...
        # Loop through and append bridges to the bridges list
        for i in data:
            if i[3] == 'bridge':
                bridges.append(i)
    ...
    def get_list_cameras(self):
        ...
        # Loop through and append cameras to the cameras list
        for i in data:
            if i[3] == 'camera':
                cameras.append(i)
        ...
        for camera in cameras:
            ...
            guid = camera[8]
            status = camera[4][0][1]
            print(f'ID: {id} | Name: {name} | GUID: {guid} | Status: {status}')

    def add_bridge(self, name: str, connectID: str):
        # Define payload and make PUT request
    ...
    def delete_bridge(self, id: str):
        # Define payload and make DELETE request
        payload = {"id": id}
        response = self.rs.delete(self.base_url + "/g/device", data = payload)
        assert response.status_code == 200
    
    def add_camera(self, name: str, settings: json):
        # Define payload and make PUT request
